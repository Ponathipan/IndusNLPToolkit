...
class HindiTextCleaner:
    ...
    def translit_english(self, token):
        ...
        Returns:
        - str: The transliterate token.
        """
        match = re.search("[a-zA-Z]", token)
        if match:
            return self.transliterator.transform(token)
        else:
            return token
    ...
    def remove_non_hindi_sentences(self, line):
        ...
        """
        non_hindi_chars = re.findall("[^ऀ-ॿ0-9\s]", line)
        hindi_chars = re.findall("[ऀ-ॿ0-9\s]", line)
...
if __name__ == "__main__":
    ...
    sample_text = """४ ३.1४16 + ८१० ३.1४16 संसद के विशेष सत्र (Parliament Special Session) के बीच कल यानी, सोमवार, 18 सितंबर को प्रधानमंत्री नरेंद्र मोदी (PM Narendra Modi) की अध्यक्षता में हुई केंद्रीय कैबिनेट बैठक हुई जिसमें  महिला आरक्षण बिल (Women's Reservation Bill) मंजूरी दे दी गई है. 
